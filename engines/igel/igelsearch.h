...
#ifndef IGEL_SEARCH_H
#define IGEL_SEARCH_H
...
public:
    IgelSearch();
    ~IgelSearch();
    IgelSearch(const IgelSearch&) = delete;
    IgelSearch& operator=(const IgelSearch&) = delete;
...
private:
    ...
    bool isGameOver(IgelPosition & pos, std::string & result, std::string & comment, Move & bestMove, int & legalMoves);
    void printPV(const IgelPosition& pos, int iter, int selDepth, EVAL score, const Move* pv, int pvSize, Move mv, uint64_t sumNodes, uint64_t sumHits, uint64_t nps);
    bool isDraw();
...
private:
    ...
    std::unique_ptr<IgelSearch[]> m_threadParams;
