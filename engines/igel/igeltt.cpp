...
IgelTTable::IgelTTable() : m_hash(nullptr), m_hashSize(0), m_hashAge(0)
{
}

IgelTTable & IgelTTable::instance()
{
    static IgelTTable instance;
    return instance;
}

void IgelTTable::record(Move mv, EVAL score, I8 depth, int ply, U8 type, U64 hash0)
{
    assert(m_hash);
    assert(m_hashSize);
...
bool IgelTTable::retrieve(U64 hash, TEntry & hentry)
{
    assert(m_hash);
    assert(m_hashSize);
...
bool IgelTTable::clearHash(unsigned int threads)
{
    if (!m_hash)
        return false;
...
bool IgelTTable::setHashSize(double mb, unsigned int threads)
{
    if (!mb)
        return false;
...
bool IgelTTable::increaseAge()
{
    ++m_hashAge;
    return true;
}

void IgelTTable::clearAge()
{
    m_hashAge = 0;
}

void IgelTTable::prefetchEntry(U64 hash)
{
    assert(hash);
    assert(m_hash);
    assert(m_hashSize);
