...
int Uci::handleCommands()
{
    ...
    if (!IgelTTable::instance().setHashSize(DEFAULT_HASH_SIZE, DEFAULT_THREADS)) {
...
void Uci::handleCommands(const std::string& cmd)
{
...
void Uci::onUciNewGame()
{
  ...
  IgelTTable::instance().clearHash(m_searcher.getThreadsCount());
  IgelTTable::instance().clearAge();
...
void Uci::onGo(commandParams params)
{
  ...
  IgelTTable::instance().increaseAge();
...
int Uci::onBench(const char * nnue)
{
    ...
    if (!Eval::IGEL_NNUE::load_eval_file(fileNNUE)) {
    ...
    if (!IgelTTable::instance().setHashSize(16, 1)) {
...
{ 
    ...
    if (name == "Hash" || name == "hash") {
        if (!IgelTTable::instance().setHashSize(atoi(value.c_str()), m_searcher.getThreadsCount())) {
            std::cout << "Fatal error: unable to allocate memory for transposition table" << std::endl;
            exit(1);
        }
    }
    ...
    else if (name == "Skill Level" || name == "skill level") {
        ...
        if (!Eval::IGEL_NNUE::load_eval_file(value)) {
...
#endif
    else if (name == "Ponder" || name == "ponder")
