...
U64 IgelPosition::s_hash[64][14];
U64 IgelPosition::s_hashSide[2];
U64 IgelPosition::s_hashCastlings[256];
U64 IgelPosition::s_hashEP[256];
U32 IgelPosition::s_pawnHash[64][14];

const int IgelPosition::s_matIndexDelta[14] = { 0, 0, 0, 0, 3, 3, 3, 3, 5, 5, 10, 10, 0, 0 };

bool IgelPosition::CanCastle(COLOR side, U8 flank) const
{
    if (InCheck())
        return false;
...
std::string IgelPosition::FEN() const
{
...
U64 IgelPosition::GetAttacks(FLD to, COLOR side, U64 occ) const
{
    U64 att = 0;
...
bool IgelPosition::IsAttacked(FLD f, COLOR side) const
{
...
#if defined(EVAL_NNUE)
inline PieceId IgelPosition::piece_id_on(Square sq) const
{
...
bool IgelPosition::MakeMove(Move mv)
{
...
void IgelPosition::MakeNullMove()
{
    Undo & undo = m_undos[m_undoSize++];
    undo.m_castlings = m_castlings;
    undo.m_ep = m_ep;
...
void IgelPosition::UnmakeNullMove()
{
    if (m_undoSize == 0)
        return;
...
void IgelPosition::MovePiece(PIECE p, FLD from, FLD to)
{
...
#if defined(EVAL_NNUE)
inline Piece IgelPosition::NNUEPieceAdaptor(PIECE p)
{
    Piece t = NO_PIECE;
...
void IgelPosition::Put(FLD f, PIECE p)
{
    assert(f >= 0 && f < 64);
    assert(p >= 2 && p < 14);
...
#if defined(EVAL_NNUE)
void IgelPosition::Put(FLD f, PIECE p, PieceId & next_piece_id)
{
    assert(f >= 0 && f < 64);
    assert(p >= 2 && p < 14);
...
void IgelPosition::Remove(FLD f)
{
    assert(f >= 0 && f < 64);
    PIECE p = m_board[f];
    assert(p >= 2 && p < 14);
...
int IgelPosition::Repetitions() const
{
    int r = 1;
    U64 hash0 = Hash();
...
bool IgelPosition::SetFEN(const std::string& fen)
{
...
void IgelPosition::SetInitial()
{
    SetFEN(STD_POSITION);
}

bool IgelPosition::isInitialPosition()
{
    return m_initialPosition;
}

bool IgelPosition::NonPawnMaterial()
{
    COLOR side = Side();
...
EVAL IgelPosition::nonPawnMaterial()
{
    return nonPawnMaterial(WHITE)
        +  nonPawnMaterial(BLACK);
}

EVAL IgelPosition::nonPawnMaterial(COLOR side)
{
...
#if defined(EVAL_NNUE)
const EvalList * IgelPosition::eval_list() const {
    return &evalList;
}
#endif
