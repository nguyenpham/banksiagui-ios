...
//            guiCom << "info depth " + to_string(depth) + " currmove " + m->toString() + " currmovenumber " + to_string(i + 1) + "\n";
            ...
            std::string s = "info depth " + to_string(depth) + " currmove " + m->toString() + " currmovenumber " + to_string(i + 1) + "\n";
...
static void uciScore(searchthread *thr, int inWindow, U64 thinktime, int score, int mpvIndex = 0)
{
    ...
    {
        s = "info depth " + to_string(thr->depth) + " seldepth " + to_string(pos->seldepth) + " multipv " + to_string(mpvIndex + 1) + " time " + to_string(thinktime * 1000 / en.frequency)
            ...
            if (measuredOverhead > en.maxMeasuredEngineOverhead) {
                ...
                if (measuredOverhead > engineOverhead / 2)
                {
                    ...
                    if (measuredOverhead > engineOverhead)
                        s = "info string Measured engine overhead is " + to_string(measuredOverhead) + "ms and time forfeits are very likely. Please increase Move_Overhead option!\n";
                    else
                        s = "info string Measured engine overhead is " + to_string(measuredOverhead) + "ms (> 50% of allowed via Move_Overhead option).\n";
