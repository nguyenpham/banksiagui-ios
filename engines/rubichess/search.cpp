...
int chessposition::rootsearch(int alpha, int beta, int depth, int inWindowLast)
{
    ...
    for (int i = 0; i < rootmovelist.length; i++)
    {
        ...
        {
            char s[256];
            sprintf_s(s, "info depth %d currmove %s currmovenumber %d", depth, m->toString().c_str(), i + 1);
//            cout << s;
          engine_message(rubichess, s);
        }
#endif
        int reduction = 0;
...
//        cout << "bestmove " + strBestmove + strPonder + "\n";
        engine_message(rubichess, "bestmove " + strBestmove + strPonder);
