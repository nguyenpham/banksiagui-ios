...
{
    useTb = min(Rubi_TBlargest, en.SyzygyProbeLimit);
    tbPosition = 0;
    useRootmoveScore = 0;
...
{
    rubi_init_tablebases((char*)en.SyzygyPath.c_str());
}
...
{
  static string fen = STARTFEN;
  static vector<string> moves;
  static vector<string> searchmoves;
  static vector<string> commandargs;
  static GuiToken command = UNKNOWN;
  static size_t ci, cs;
  static bool bGetName, bGetValue;
  static string sName, sValue;
  static bool bMoves;
  ...
  bool pendingposition = (inputstring == "");
    ...
    do
    {
        if (stopLevel >= ENGINESTOPIMMEDIATELY || inputstring == "")
        {
            searchWaitStop();
        }
        if (inputstring == "")
