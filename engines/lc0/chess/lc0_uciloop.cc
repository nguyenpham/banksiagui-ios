...
// Added by BanksiaGUI
void UciLoop::RunCmd(const std::string& line)
{
  try {
    auto command = ParseCommand(line);
    // Ignore empty line.
    if (command.first.empty()) return;
    if (!DispatchCommand(command.first, command.second)) return;
  } catch (Exception& ex) {
    SendResponse(std::string("error ") + ex.what());
  }
}
