...
namespace {
  ...
  template<bool Root>
  uint64_t sf_perft(Position& pos, Depth depth) {
    ...
    for (const auto& m : MoveList<LEGAL>(pos))
    {
        ...
        {
            pos.do_move(m, st);
            cnt = leaf ? MoveList<LEGAL>(pos).size() : sf_perft<false>(pos, depth - 1);
            nodes += cnt;
            pos.undo_move(m);
        }
      if (Root) {
//            sync_cout << UCI::move(m, pos.is_chess960()) << ": " << cnt << sync_endl;
        engine_message(stockfish, UCI::move(m, pos.is_chess960()) + ": " + std::to_string(cnt));
      }
    }
    return nodes;
  }
...
void MainThread::search() {
  ...
  if (Limits.perft)
  {
      nodes = sf_perft<true>(rootPos, Limits.perft);
//      sync_cout << "\nNodes searched: " << nodes << "\n" << sync_endl;
    engine_message(stockfish, "\nNodes searched: " + std::to_string(nodes));
      return;
  }
...
//                << sync_endl;
    engine_message(stockfish, "info depth 0 score " + UCI::value(rootPos.checkers() ? -VALUE_MATE : VALUE_DRAW));
...
//      sync_cout << UCI::pv(bestThread->rootPos, bestThread->completedDepth, -VALUE_INFINITE, VALUE_INFINITE) << sync_endl;
    engine_message(stockfish, UCI::pv(bestThread->rootPos, bestThread->completedDepth, -VALUE_INFINITE, VALUE_INFINITE));
...
//                  sync_cout << UCI::pv(rootPos, rootDepth, alpha, beta) << sync_endl;
                engine_message(stockfish, UCI::pv(rootPos, rootDepth, alpha, beta));
...
//                    << " currmovenumber " << moveCount + thisThread->pvIdx << sync_endl;
        ...
        if (Eval::nnueHits && Eval::evalHits) {
          s += " nnuehits " + std::to_string(static_cast<int>(Eval::nnueHits * 1000 / Eval::evalHits));
        }
        engine_message(stockfish, s);
      }
...
string UCI::pv(const Position& pos, Depth depth, Value alpha, Value beta) {
  ...
  for (size_t i = 0; i < multiPV; ++i)
  {
    ...
    if (Eval::nnueHits && Eval::evalHits) {
      ss << " nnuehits " << static_cast<int>(Eval::nnueHits * 1000 / Eval::evalHits);
    }
