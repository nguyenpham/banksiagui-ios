    ...
    for (int piece = PAWN; piece <= KING; piece++)
        for (int sq = 0; sq < SQUARE_NB; sq++)
            for (int colour = WHITE; colour <= BLACK; colour++)
                ZobristKeys[makePiece(piece, colour)][sq] = ethereal_rand64();

    // Init the Zobrist keys for each enpass file
    for (int file = 0; file < FILE_NB; file++)
        ZobristEnpassKeys[file] = ethereal_rand64();

    // Init the Zobrist keys for each castle rook
    for (int sq = 0; sq < SQUARE_NB; sq++)
        ZobristCastleKeys[sq] = ethereal_rand64();

    // Init the Zobrist key for side to move
    ZobristTurnKey = ethereal_rand64();
}
