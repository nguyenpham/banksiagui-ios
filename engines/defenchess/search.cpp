...
std::string pv_toString(Position *p, Metadata *md) {
  std::string s;
    int i = 0;
    while (md->pv[i] != no_move) {
        s += move_to_str(p, md->pv[i++]) + " ";
    }
  return s;
}
...
//    std::cout << std::endl;
  ...
  std::string s =
  "info depth " + std::to_string(depth) + " seldepth " + std::to_string((my_thread->selply + 1)) + " multipv 1 " +
  "tbhits " + std::to_string(tb_hits) + " score ";

  if (score <= MATED_IN_MAX_PLY) {
      s += "mate " + std::to_string((-MATE - score) / 2 + 1);
  } else if (score >= MATE_IN_MAX_PLY) {
    s += "mate " + std::to_string((MATE - score) / 2 + 1);
  } else {
    s += "cp " + std::to_string(score * 100 / PAWN_END);
  }
  ...
  s += " hashfull " + std::to_string(hashfull());

  uint64_t nodes = sum_nodes();
  s += " nodes " + std::to_string(nodes) + " nps " + std::to_string(nodes*1000/(time_taken+1)) + " time " + std::to_string(time_taken) + " pv ";
  if (pv_printed) {
      s += pv_toString(p, md);
  } else {
      // For fail lows, only print the first move of the main pv
    s += move_to_str(p, main_pv[0]);
  }
  engine_message(defenchess, s);
}
...
//            std::cout << "bestmove " << move_to_str(p, tb_move) << std::endl;
          ...
          engine_message(defenchess, "info score cp 0" );
          engine_message(defenchess, "bestmove " + move_to_str(p, tb_move));
            return;
        }
...
//            std::cout << "bestmove " << move_to_str(p, *root_moves.begin()) << std::endl;
          engine_message(defenchess, "bestmove " + move_to_str(p, *root_moves.begin()));
            return;
        }
        if (root_moves.size() == 0) {
            while (is_pondering) {}
//            std::cout << "bestmove none" << std::endl;
          engine_message(defenchess, "bestmove none");
            return;
        }
    }
...
//    std::cout << std::endl;
  ...
  engine_message(defenchess, "info time " + std::to_string(time_passed()));
  std::string s = "bestmove " + move_to_str(p, main_pv[0]);
  if (main_pv[0] == latest_pv && is_move_valid(latest_ponder)) {
      s += " ponder " + move_to_str(p, latest_ponder);
  }
  engine_message(defenchess, s);
