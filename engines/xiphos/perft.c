...
uint64_t xiphos_perft(search_data_t *sd, int depth, int ply, int additional_tests)
{
  ...
  for (i = 0; i < moves_cnt; i ++)
  {
    ...
    nodes += xiphos_perft(sd, depth - 1, ply + 1, additional_tests);
    undo_move(sd);
  }
...
void run_tests()
{
  ...
  for (i = 0; i < sizeof(tests) / sizeof(test_t); i ++)
  {
    ...
    nodes = xiphos_perft(sd, tests[i].depth, 0, 1);
