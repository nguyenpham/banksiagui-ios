...
static int qsearch(search_data_t *sd, int pv_node, int alpha, int beta, int depth, int ply)
{
        ...
        int depth, int ply, int use_pruning, move_t skip_move)
{
  ...
  // probe tablebases
  if (XIPHOS_TB_LARGEST > 0 && !root_node && !pos->fifty_cnt && !pos->c_flag)
  {
    piece_cnt = _popcnt(_occ(pos));
    if (piece_cnt < XIPHOS_TB_LARGEST ||
       (piece_cnt == XIPHOS_TB_LARGEST && depth >= search_settings.tb_probe_depth))
    {
...
void print_best_move(search_data_t *sd)
{
  ...
  char buf[256];
  sprintf(buf, "bestmove %s", m_to_str(best_move));
  if (_is_m(ponder_move))
    sprintf(buf + strlen(buf), " ponder %s", m_to_str(ponder_move));
//  _p("\n");
  engine_message_c(xiphos, buf);
}
...
void *xiphos_search()
{
  ...
  // probe tablebases
  if (XIPHOS_TB_LARGEST > 0 && !sd->pos->c_flag && _popcnt(_occ(sd->pos)) <= XIPHOS_TB_LARGEST)
  {
