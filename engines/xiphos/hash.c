...
void init_z_keys()
{
  ...
  for (i = 0; i < BOARD_SIZE; i ++)
    for (j = 0; j < Z_KEYS_MAX_INDEX; j ++)
      shared_z_keys.positions[i][j] = xiphos_rand64();

  for (i = 0; i < C_FLAG_MAX; i ++)
    shared_z_keys.c_flags[i] = xiphos_rand64();

  shared_z_keys.side_flag = xiphos_rand64();
}
