...
int starts_with(const char *buf, const char *str)
{
  return !strncmp(buf, str, strlen(str));
}

int _cmd_cmp(const char *buf, const char *cmd_str)
{
...
void read_fen(search_data_t *sd, const char *buf)
{
  position_t *pos;
  char c, *moves_buf;
  int i, sq, side;
...
void uci_position_startpos(const char *buf)
{
...
void parse_go_cmd(const char *buf)
{
...
void uci_perft(const char *buf)
{
  ...
  time_ms = time_in_ms();
  nodes = xiphos_perft(search_settings.sd, depth, 0, 0);
...
void uci_info(move_t *pv)
{
  ...
  char buf2[BUFFER_LINE_SIZE];
  sprintf(buf2, "info depth %d score %s nodes %"PRIu64" tbhits %"PRIu64" time %"PRIu64" nps %"PRIu64" ",
  ...
  sprintf(buf2 + strlen(buf2), "%s", pv_string);
  engine_message_c(xiphos, buf2);
}
...
void set_ponder(const char *buf)
{
...
static pthread_t main_search_thread;
...
void xiphos_uci_cmd(const char* str)
{

    if (_cmd_cmp(str, CMD_UCI))
    {
      _p("id name %s %s\n", VERSION, ARCH);
      _p("id author %s\n", AUTHOR);
    ...
    if (_cmd_cmp(str, CMD_READY))
      _p("readyok\n");

    else if (_cmd_cmp(str, CMD_QUIT))
      return;

    else if (_cmd_cmp(str, CMD_STOP))
    {
    ...
    else if (_cmd_cmp(str, CMD_PONDERHIT))
    {
      pthread_mutex_lock(&search_settings.mutex);
      search_status.go.ponder = 0;
    ...
    else if (_cmd_cmp(str, CMD_NEW_GAME))
    {
      full_reset_search_data();
      read_fen(search_settings.sd, initial_fen);
    }

    else if (_cmd_cmp(str, CMD_POSITION_FEN))
      read_fen(search_settings.sd, str);

    else if (_cmd_cmp(str, CMD_POSITION_STARTPOS))
      uci_position_startpos(str);

    else if (_cmd_cmp(str, CMD_PERFT))
      uci_perft(str);

//    else if (_cmd_cmp(&buf, CMD_TEST))
//      run_tests();

    else if (_cmd_cmp(str, CMD_PRINT))
      print_board(search_settings.sd->pos);

    else if (_cmd_cmp(str, OPTION_HASH))
      set_hash_size(atoi(str));

    else if (_cmd_cmp(str, OPTION_THREADS))
      set_max_threads(atoi(str));

    else if (_cmd_cmp(str, OPTION_PONDER))
      set_ponder(str);

    else if (_cmd_cmp(str, OPTION_SYZYGY_PATH))
      set_syzygy_path(str);

    else if (_cmd_cmp(str, OPTION_SYZYGY_PROBE_DEPTH))
      set_syzygy_probe_depth(atoi(str));

    else if (_cmd_cmp(str, CMD_GO))
    {
      if (searching)
        pthread_join(main_search_thread, NULL);

      parse_go_cmd(str);
      pthread_create(&main_search_thread, NULL, xiphos_search, NULL);
      searching = 1;
    }
}
