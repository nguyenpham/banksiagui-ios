...
int eval(position_t *pos)
{
  ...
  for (side = WHITE; side < N_SIDES; side ++)
  {
    ...
    #define _score_rook_open_files                                             \
      ...
      {                                                                        \
        ...
        score_mid += xiphos_rook_file_bonus[PHASE_MID][open_file];                    \
        score_end += xiphos_rook_file_bonus[PHASE_END][open_file];                    \
      }
    ...
    {
      score_mid += xiphos_bishop_pair[PHASE_MID];
      score_end += xiphos_bishop_pair[PHASE_END];
    }
  ...
  for (side = WHITE; side < N_SIDES; side ++)
  {
    ...
    score_mid += pcnt * xiphos_pawn_mobility[PHASE_MID];
    score_end += pcnt * xiphos_pawn_mobility[PHASE_END];
