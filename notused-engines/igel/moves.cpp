...
void GenAllMoves(const IgelPosition& pos, MoveList& mvlist)
{
    mvlist.Clear();
...
void GenCapturesAndPromotions(const IgelPosition& pos, MoveList& mvlist)
{
    mvlist.Clear();
...
void AddSimpleChecks(const IgelPosition& pos, MoveList& mvlist)
{
    COLOR side = pos.Side();
    COLOR opp = side ^ 1;
...
U64 GetCheckMask(const IgelPosition& pos)
{
    COLOR side = pos.Side();
    COLOR opp = side ^ 1;
    FLD K = pos.King(side);
...
void GenMovesInCheck(const IgelPosition& pos, MoveList& mvlist)
{
    mvlist.Clear();
