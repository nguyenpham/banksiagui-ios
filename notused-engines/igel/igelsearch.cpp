...
/*static */constexpr int IgelSearch::m_lmpDepth;
/*static */constexpr int IgelSearch::m_lmpPruningTable[2][9];
/*static */constexpr int IgelSearch::m_cmpDepth[2];
/*static */constexpr int IgelSearch::m_cmpHistoryLimit[2];
/*static */constexpr int IgelSearch::m_fmpDepth[2];
/*static */constexpr int IgelSearch::m_fmpHistoryLimit[2];
/*static */constexpr int IgelSearch::m_fpHistoryLimit[2];

IgelSearch::IgelSearch() :
    m_nodes(0),
    m_tbHits(0),
    m_limitCheck(1023),
    m_t0(0),
...
EVAL IgelSearch::abSearch(EVAL alpha, EVAL beta, int depth, int ply, bool isNull, bool rootNode, Move skipMove/*= 0*/)
{
    //
    //   qsearch
    //
    ...
    IgelTTable::instance().prefetchEntry(m_position.Hash());
    ...
    if (!skipMove)
      IgelTTable::instance().record(bestMove, bestScore, depth, ply, type, m_position.Hash());

    return bestScore;
}

EVAL IgelSearch::qSearch(EVAL alpha, EVAL beta, int ply, int depth, bool isNull/* = false*/)
{
    ++m_nodes;
    m_pvSize[ply]   = 0;
    m_selDepth      = std::max(ply, m_selDepth);
  ...
  IgelTTable::instance().record(bestMove, bestScore, tteDepth, ply, type, m_position.Hash());
    return bestScore;
}

void IgelSearch::clearHistory()
{
...
int IgelSearch::extensionRequired(Move mv, Move lastMove, bool inCheck, int ply, bool onPV, size_t quietMoves, int cmhistory, int fmhistory)
{
...
bool IgelSearch::isGameOver(IgelPosition & pos, std::string & result, std::string & comment, Move & bestMove, int & legalMoves)
{
    MoveList mvlist;
...
void IgelSearch::printPV(const IgelPosition& pos, int iter, int selDepth, EVAL score, const Move* pv, int pvSize, Move mv, uint64_t sumNodes, uint64_t sumHits, uint64_t nps)
{
...
//    std::cout << std::endl;
  ...
  std::string s = "info depth " + std::to_string(iter) + " seldepth " + std::to_string(selDepth);

  if (abs(score) >= (CHECKMATE_SCORE - MAX_PLY))
      s += std::string(" score mate") + ((score >= 0) ? " " : " -") + std::to_string(((CHECKMATE_SCORE - abs(score)) / 2) + 1);
  else
    s += " score cp " + std::to_string(score);

  s += " time " + std::to_string(dt) +
    " nodes " + std::to_string(sumNodes) +
  " tbhits " + std::to_string(sumHits);

  if (nps)
    s += " nps " + std::to_string(nps);

  s += " pv";

  if (pvSize > 0) {
      for (int i = 0; i < pvSize; ++i)
    s += " " + MoveToStrLong(pv[i]);
  }
  else
    s += " " + MoveToStrLong(mv);

  engine_message(igel, s);
}

bool IgelSearch::ProbeHash(TEntry & hentry)
{
    auto hash  = m_position.Hash();
    return IgelTTable::instance().retrieve(hash, hentry);
}
...
void IgelSearch::startWorkerThreads(Time time)
{
    for (unsigned int i = 0; i < m_thc; ++i) {
...
void IgelSearch::indicateWorkersStop()
{
    for (unsigned int i = 0; i < m_thc; ++i) {
...
void IgelSearch::stopWorkerThreads()
{
    indicateWorkersStop();
...
void IgelSearch::waitUntilCompletion()
{
...
void IgelSearch::isReady()
{
    indicateWorkersStop();
    m_flags |= TERMINATED_BY_USER;
    std::unique_lock<std::mutex> lk(m_readyMutex);
//    std::cout << "readyok" << std::endl;
  engine_message(igel, "readyok");
}

void IgelSearch::stopPrincipalSearch()
{
    m_ponderHit = false;
    m_flags |= TERMINATED_BY_USER;
}

void IgelSearch::startPrincipalSearch(Time time, bool ponder)
{
    ...
    if (!m_principalThread)
        m_principalThread.reset(new std::thread(&IgelSearch::lazySmpSearcher, this));
}

uint64_t IgelSearch::startSearch(Time time, int depth, bool ponderSearch, bool bench)
{
    m_iterPVSize = 0;
    m_nodes = 0;
    m_selDepth = 0;
    ...
    auto printBestMove = [](IgelSearch * pthis, IgelPosition & pos, Move m, Move p) {
...
//        std::cout << std::endl;
      std::string s;
      if (m)
          s += "bestmove " + MoveToStrLong(m);

      if (p)
        s += " ponder " + MoveToStrLong(p);
      engine_message(igel, s);
    };
...
void IgelSearch::setSyzygyDepth(int depth)
{
    m_syzygyDepth = depth;
}

void IgelSearch::setLevel(int level)
{
    m_level = level;
}

void IgelSearch::setThreadCount(unsigned int threads)
{
    if (threads == m_thc)
        return;
    ...
    m_threadParams.reset(new IgelSearch[threads]);

    for (unsigned int i = 0; i < m_thc; ++i)
        m_threads[i] = std::thread(&IgelSearch::lazySmpSearcher, &m_threadParams[i]);
}

unsigned int IgelSearch::getThreadsCount()
{
    return m_thc + 1;
}

void IgelSearch::lazySmpSearcher()
{
    while (!m_terminateSmp)
    {
        ...
        {
            std::unique_lock<std::mutex> lk(m_readyMutex);
            m_lazycv.wait(lk, std::bind(&IgelSearch::getIsLazySmpWork, this));
...
void IgelSearch::releaseHelperThreads()
{
    for (unsigned int i = 0; i < m_thc; ++i)
    {
...
bool IgelSearch::setFEN(const std::string& fen)
{
    for (unsigned int i = 0; i < m_thc; ++i)
...
bool IgelSearch::setInitialPosition()
{
    for (unsigned int i = 0; i < m_thc; ++i)
...
bool IgelSearch::makeMove(Move mv)
{
    for (unsigned int i = 0; i < m_thc; ++i)
