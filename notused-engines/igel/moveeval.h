...
public:
    ...
    static bool isSpecialMove(const Move & mv, IgelSearch * pSearch);
    static bool isGoodCapture(const Move & mv);
    static void sortMoves(IgelSearch * pSearch, MoveList & mvlist, Move hashMove, int ply);
    static Move getNextBest(MoveList & mvlist, size_t i);
    static EVAL SEE_Exchange(IgelSearch * pSearch, FLD to, COLOR side, EVAL currScore, EVAL target, U64 occ);
    static EVAL SEE(IgelSearch * pSearch, const Move & mv);
